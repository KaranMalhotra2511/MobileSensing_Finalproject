...
public class BloomFilter<E> implements Serializable {
    ...
    /**
     * Creates an empty Bloom filter with a given false positive probability.
     * The number of bits per element and the number of hash functions is estimated to match the
     * false positive probability. The size of the bit set is restricted by the given range.
     * @param minBitSetSize
     * @param maxBitSetSize
     * @param expectedNumberOfElements
     * @param falsePositiveProbability
     */
    public static BloomFilter createBloomFilter(int minBitSetSize, int maxBitSetSize, int expectedNumberOfElements, double falsePositiveProbability) {
        int size = (int)(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2)) * expectedNumberOfElements;
        if (size < minBitSetSize)
            size = minBitSetSize;
        if (size > maxBitSetSize)
            size = maxBitSetSize;
        return new BloomFilter(size, expectedNumberOfElements);
    }

    /**
     * Encode a BloomFilter into a bytes array.
     */
    public byte[] encodeWire() {
        byte[] bitSetWire = bitset.toByteArray();

        ByteBuffer bb = ByteBuffer.allocate(3 * Integer.SIZE + + bitSetWire.length);
        bb.putInt(bitSetSize);
        bb.putInt(expectedNumberOfFilterElements);
        bb.putInt(numberOfAddedElements);
        bb.put(bitSetWire);
        return bb.array();
    }

    /**
     * Decode a BloomFilter from a bytes array.
     * @param wire
     * @return
     */
    public static BloomFilter decodeWire(byte[] wire) {
        ByteBuffer bb = ByteBuffer.wrap(wire);
        int bitSetSize = bb.getInt();
        int expectedNumberOfElement = bb.getInt();
        int numberOfElement = bb.getInt();
        byte[] bitSetWire = new byte[wire.length - 3 * Integer.SIZE];
        bb.get(bitSetWire);
        BitSet bitSet = BitSet.valueOf(bitSetWire);
        return new BloomFilter(bitSetSize, expectedNumberOfElement, numberOfElement, bitSet);
    }
